...
export default function Header() {
  ...
  const totalPrice = cart.reduce((acc, item) => acc + (item.details?.price || 0) * item.quantity, 0);
  ...
  return (
    <header className="w-full font-sans sticky top-0 z-50 bg-white shadow-sm">
      ...
      <div className="container mx-auto py-3 md:py-5 px-4 flex items-center justify-between">
        ...
        <div className="w-[100px] md:w-[120px] cursor-pointer" onClick={() => router.push("/")}>
        ...
        <div className="hidden md:flex flex-1 max-w-[500px] border-[1px] border-gray-300 rounded-sm overflow-hidden mx-10">
          ...
          <input type="text" placeholder="Search for furniture..." className="w-full p-2 text-[14px] outline-none" />
  ...
  <div>
    ...
    <p className="text-gray-400 text-[12px] uppercase tracking-tighter">
      Karachi & Beyond
          ...
          {/* Cart Section */}
          <div onClick={() => router.push("/cart")} className="flex items-center md:bg-[#D61F26] md:text-white md:p-3 gap-2 md:gap-4 md:min-w-[210px] rounded-sm cursor-pointer">
            ...
            <div className="hidden md:block">
              <p className="text-[11px] font-light opacity-90">Rs.{totalPrice.toLocaleString()}</p>
      ...
      <div className="hidden md:block relative bg-[#454545]">
        <div className="container mx-auto flex justify-between items-center pr-6 w-full">
          ...
          <div className="flex items-center gap-4 text-gray-300">
            <Facebook size={16} className="hover:text-[#D61F26] cursor-pointer" />
            <Twitter size={16} className="hover:text-[#D61F26] cursor-pointer" />
            <Instagram size={16} className="hover:text-[#D61F26] cursor-pointer" />
            <Youtube size={16} className="hover:text-[#D61F26] cursor-pointer" />
          </div>
        </div>
      </div>
      ...
      >
        ...
        <nav className="flex flex-col text-white overflow-y-auto">
          ...
          {categories.map((cat) => {
            ...
            const hasSubItems = (cat.links && cat.links.length > 0) || (cat.sections && cat.sections.length > 0);
            ...
            return (
              <div key={cat.id} className="border-b border-gray-800">
                <div
                  ...
                  onClick={() => (hasSubItems ? toggleCategory(cat.id) : router.push(`/category/${cat.id}`))}
                >
                  {cat.title}
                  {hasSubItems && (
                    <ChevronDown
        ...
        <div className="mt-auto p-4 bg-[#1a1a1a]">
          ...
          >
            ...
            <span className="text-black text-[13px] font-semibold">Need Help? Chat with us</span>
          </a>
        </div>
      </div>
    </header>
